...
def getdata():
    ...
    # Get first scroll id
    try:
        r = requests.post('https://gracc.opensciencegrid.org/q/gracc.osg.raw*/_search?search_type=scan&scroll=1m', data=q)
    except requests.ConnectionError as e:
        print e
        sys.exit(1)
    if not r.status_code == requests.codes.ok:
        print 'Error accessing ElasticSearch'
        r.raise_for_status()
    scroll_id = r.json()['_scroll_id']
    while True:
        try:
            r = requests.post('https://gracc.opensciencegrid.org/q/_search/scroll?scroll=1m', data=scroll_id)
        except requests.ConnectionError as e:
            print e
            sys.exit(1)
        ...
        if not r.json()['hits']['hits']:
            break
        # try:
        #     d = requests.delete('https://gracc.opensciencegrid.org/q/_search/scroll', data=scroll_id)
    # requests.delete('https://gracc.opensciencegrid.org/q/_search/scroll', data=scroll_id)
